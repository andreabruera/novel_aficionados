...
### NOVELS EDIT: added the sentence_count argument, needed for keeping track of the current sentence i.e. to make sure the alpha is adjusted accordingly to the current training point.
def train_batch_sg(model, sentences, sentence_count, alpha, work=None, compute_loss=False):
    result = 0
    window = model.window
                ...
                if pos2 != pos:
                    ...
                    if model.wv.index2word[word2.index] == \
                     model.vocabulary.nonce:
                        ...
                        ### NOVELS EDIT: had to change the argument word2.index to len(model.wv.vocab)-1, otherwise it would keep asking for an out-of-index value, corresponding to the length of the vocabulary
                        train_sg_pair(
                            model, model.wv.index2word[word.index],
...
class Nonce2Vec(Word2Vec):
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        ### NOVELS EDIT: added the sentence_count argument to the train_batch_sg function, in order to consider it when training
        tally += train_batch_sg(self, sentences, sentence_count, alpha, work) 
