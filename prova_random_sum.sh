    ...
    for window_decay in ${WINDOW_DECAY[@]};
        ...
        for subsampling_decay in ${SUBSAMPLING_DECAY[@]};
            ...
            for subsampling in ${SUBSAMPLING[@]};
                ...
                FOLDER='random_sum_Lambda_'${lambda}'_Window_decay_'${window_decay}'_Subsampling_decay_'${subsampling_decay}'_Subsampling_'${subsampling}
                echo 'Created folder:' ${FOLDER}
                for SIX_NOVELS in $(ls ${DOWNLOADED_NOVELS_FOLDER});
                    do
                    TRAINING_FOLDER=random_sum_test_novels_part_${SIX_NOVELS}/${FOLDER}
                    ...
                    for NOVEL_FOLDER in $(ls ${TRAINING_FOLDER});
                        do
                        ./prova_random_sum_individuale.sh ${TRAINING_FOLDER} ${NOVEL_FOLDER} ${lambda} ${window_decay} ${subsampling_decay} ${subsampling} &  
                        done
                    wait
                    done
                wait
