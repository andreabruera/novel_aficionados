#!/bin/bash
#This script runs the novel preparation pipeline on one wikipedia page.
...
mkdir ${ORIGINAL_FOLDER}/quality_test
WIKI_FOLDER=/mnt/cimec-storage-sata/users/andrea.bruera/novel_aficionados/${ORIGINAL_FOLDER}

python3 quality_test/extract_wiki_characters.py ${ORIGINAL_FOLDER} --number ${BOOK_NUMBER}

n2v test --on wiki_novels --model /mnt/cimec-storage-sata/users/andrea.bruera/wiki_training/data/wiki_w2v_2018_size400_max_final_vocab250000_sg1 --folder ${WIKI_FOLDER} --data ${BOOK_NUMBER} --alpha 1 --neg 3 --window 15 --sample ${subsampling} --epochs 1 --lambda ${lambda} --sample-decay ${subsampling_decay} --window-decay ${window_decay} --simil_out --sum_only > /dev/null 
