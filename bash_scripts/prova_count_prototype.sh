    ...
    do
        for window_size in ${WINDOW_SIZES[@]};
            ...
            for SIX_NOVELS in $(ls ${DOWNLOADED_NOVELS_FOLDER});
                do
                TRAINING_FOLDER=${TRAINING_MODE}_test_novels_part_${SIX_NOVELS}/${FOLDER}_window${window_size}_context${context}_weight${weight}
                ...
                for NOVEL_FOLDER in $(ls ${TRAINING_FOLDER});
                    do
                    ./bash_scripts/prova_${FOLDER}_individuale.sh ${TRAINING_FOLDER} ${NOVEL_FOLDER} ${window_size} ${context} ${weight}  
                    done
                wait
                done
            done
