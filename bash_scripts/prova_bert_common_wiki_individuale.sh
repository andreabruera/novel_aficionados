...
echo 'Training WIKI on novel' ${BOOK_NUMBER}

FULL_FOLDER=/mnt/cimec-storage-sata/users/andrea.bruera/novel_aficionados/${ORIGINAL_FOLDER}

n2v test --on wiki_bert_novels --model /mnt/cimec-storage-sata/users/andrea.bruera/wiki_training/data/wiki_w2v_2018_size400_window10_negative20_max_final_vocab250000_sg1 --folder ${FULL_FOLDER} --data ${BOOK_NUMBER} --write_to_file --common_nouns > /dev/null 2>&1   
